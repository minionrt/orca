...
use crate::llm::{LLM, MessageRole, Completion};
...
impl Memory {
    ...
    pub fn new(api_key: &str, base_url: &Url) -> Self {
        Memory {
            history: "".to_string(),
            llm: LLM::full(api_key.to_string(), base_url.clone(), Model::Basic.into(), None,None), //TODO replace model with one from model enum
    ...
    pub fn with_model(mut self, model: Model) -> Self {
        ...
        self.llm = LLM::full(self.api_key.clone(), self.base_url.clone(), model.into(),None,None); //add model.into() here
        self
    }
